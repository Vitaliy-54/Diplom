<x-app-layout>
    ...
    <div class="py-6 px-4 sm:px-6">
        <div class="max-w-7xl mx-auto">
            ...
            <!-- Info Banner -->
            <div class="bg-blue-200 dark:bg-blue-900/30 rounded-lg p-4 mb-6 border border-blue-400 dark:border-blue-800">
                <div class="flex items-start gap-3">
            ...
            <div class="bg-white dark:bg-gray-800 overflow-hidden shadow-sm sm:rounded-lg">
                <div class="p-6 text-gray-900 dark:text-gray-100">
                        ...
                        <div class="space-y-6">
                            ...
                            <div class="bg-gray-300/70 dark:bg-gray-900 rounded-lg p-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        ...
                                        <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $inputData['i7n'] ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-700 dark:text-gray-400 mb-1">Энергии (cm⁻¹):</p>
                                    <div class="flex flex-wrap gap-2">
                                        @if(isset($inputData['i7e']) && is_array($inputData['i7e']))
                                        @foreach($inputData['i7e'] as $index => $energy)
                                        @php
                            ...
                            <div class="bg-gray-300/70 dark:bg-gray-900 rounded-lg p-4">
                                ...
                                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                    <div>
                                        <p class="text-sm text-gray-700 dark:text-gray-400 mb-1">Количество компонент:</p>
                                        <p class="text-lg font-semibold text-gray-900 dark:text-white">{{ $inputData['i8n'] ?? '-' }}</p>
                                    </div>
                                    <div>
                                        <p class="text-sm text-gray-700 dark:text-gray-400 mb-1">Энергии (cm⁻¹):</p>
                                    <div class="flex flex-wrap gap-2">
                                        @if(isset($inputData['i8e']) && is_array($inputData['i8e']))
                                        @foreach($inputData['i8e'] as $index => $energy)
                                        @php
                                            $value = floatval($energy);
                                            $formatted = $value == floor($value) 
                                                ? number_format($value, 0, '.', '') 
                        ...
                        <div class="space-y-4">
                            ...
                            <div class="bg-gray-200 dark:bg-gray-700 rounded-lg p-6">
                                ...
                                <div class="bg-white dark:bg-gray-800 rounded-lg p-4 mb-6 border-l-4 border-orange-500">
                                    <p class="text-sm text-gray-700 dark:text-gray-400 mb-1">Параметр Fav</p>
                                    @php
                                    $favOpt = $optimizationResult['fav'] ?? 0;
                                    $favOrig = $inputOriginal['fav'] ?? $inputData['fav'] ?? 0;
                                    @endphp
                                    <p class="text-xl font-mono font-bold text-orange-700 dark:text-orange-300">
                                        {{ rtrim(rtrim(sprintf('%.15F', (float)$favOpt), '0'), '.') }}
                                    </p>
                                    <p class="text-xs text-gray-700 dark:text-gray-400 mt-1">
                                        Оригинал: {{ rtrim(rtrim(sprintf('%.15F', (float)$favOrig), '0'), '.') }}
                                    </p>
                                </div>
                                ...
                                <div class="overflow-x-auto">
                                    <table class="min-w-full divide-y divide-gray-200 dark:divide-gray-600">
                                        ...
                                        <tbody class="bg-white dark:bg-gray-800 divide-y divide-gray-200 dark:divide-gray-700">
                                            ...
                                            @php
                                            $originalFav = floatval($inputOriginal['fav'] ?? $inputData['fav'] ?? 0);
                                            ...
                                            <tr>
                                                ...
                                                <td class="px-4 py-3 text-sm font-mono text-gray-700 dark:text-gray-300 break-all">
                                                    {{ rtrim(rtrim(sprintf('%.15F', $originalFav), '0'), '.') }}
                                                </td>
                                                <td class="px-4 py-3 text-sm font-mono text-green-700 dark:text-green-300 break-all">
                                                    {{ rtrim(rtrim(sprintf('%.15F', $optimizedFav), '0'), '.') }}
                                                </td>
                    ...
                    <div class="mt-8 pt-6 border-t border-gray-200 dark:border-gray-700 text-center text-xs text-gray-700 dark:text-gray-400">
                        <p>Ho³⁺ Lifetime Calculator — Открытый расчёт</p>
            ...
            fetch('{{ route("public.calculation.copy", ["token" => $shareLink->token]) }}', {
                ...
                .then(data => {
                    if (data.success) {
                        ...
                        window.location.href = '/calculator/HO3+';
                    } else {
