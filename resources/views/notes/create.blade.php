    ...
    <div class="py-6 px-4">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 overflow-hidden hidden-until-loaded">
            <div class="bg-gray-300 dark:bg-gray-800 shadow-lg rounded-lg overflow-hidden transition-all duration-200">
                <div class="p-6">
                    <form action="{{ route('notes.store') }}" method="POST" enctype="multipart/form-data" id="note-form">
                        @csrf
                    ...
                    <div class="mb-8">
                        ...
                        <div class="mb-6">
                            ...
                            <div class="mt-1 flex justify-center px-6 pt-5 pb-6 border-2 border-gray-400 dark:border-gray-600 border-dashed rounded-md">
                                <div class="space-y-1 text-center">
                                    ...
                                    <p class="text-xs text-gray-500 dark:text-gray-400">
                                        {{ __('При создании заметки, размер всех файлов в заметке не должен превышать 20МБ') }}
                                    </p>
                                </div>
                            </div>
                            
                            <!-- Предпросмотр загруженных файлов -->
                            <div id="file-preview" class="mt-4 grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3 {{ count(old('files', [])) ? '' : 'hidden' }}"></div>
                        ...
                        <div class="mb-6">
                            <label for="is_public" class="flex items-center">
                                <input type="checkbox" name="is_public" id="is_public" value="1" {{ old('is_public') ? 'checked' : '' }} class="w-5 h-5 rounded border-gray-300 text-blue-600 focus:ring-blue-500 shadow-sm transition-colors dark:bg-gray-700 dark:border-gray-600 dark:focus:ring-blue-600">
